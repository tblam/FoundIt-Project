...
$sql = "select INTERSECTION from (select db2gse.ST_AsText(db2gse.ST_Intersection(DB2GSE.ST_Transform(a.SHAPE, 1), b.shape)) INTERSECTION from floodzone a, city_boundary b where b.name = '$city' fetch first 1000 row only) where INTERSECTION <> 'POINT EMPTY'";
